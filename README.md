...
| 腿骨朝源的位置拉回 45%（`BORU_LEG_PULL`） | 下半身不再被 X4 的宽骨架撑开 | 顶点略偏离驱动骨，走路时摆幅稍大 |
| 躯干径向放大 14%（`BORU_TORSO`） | 腰臀过渡协调（源比 X4 原版瘦） | 不是源的原始比例 |
| 头颈前移 2 cm（`BORU_HEAD_FWD`） | 脖子不再显得向前倾（源的脖子接得更靠后） | 不是源的原始头位 |
| 手指跟自己的骨 + 每节卷曲 22°（`BORU_FINGERS`/`BORU_FINGER_CURL`） | 自然半握拳（源是 T-pose 摊平的手） | 不是源的原始手型 |
